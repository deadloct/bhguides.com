...
export default function CaptureRateCalc() {
  ...
  const [encounterBonus, setEncounterBonus] = useState(
    getOptionValue(encounterOptions, encounterOptions.default)
  )
  ...
  useEffect(() => {
    const timeoutID = setTimeout(() => {
      const output =
        (cleanVal(infoCapRateRef.current) + 100) * encounterBonusRef.current
  ...
  return (
    <section id="capture-rate-calc">
      ...
      <Box flexDirection="column">
        <Box mt={VerticalSpacing}>
          <FormControl fullWidth>
            ...
            >
              {Object.entries(encounterOptions.groups).map(
                ([groupName, items]) => [
      ...
      <p className={styles.results}>
        ...
        <br />
        <span id="caprate-output" className={styles.output}>
          {output}
        </span>
      </p>
    </section>
