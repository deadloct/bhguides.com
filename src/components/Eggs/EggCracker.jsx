  ...
  function calculate() {
    ...
    for (let i = 0; i < gameCount; i++) {
      if (getRandomInt(1, fullSet + 1) <= winners) {
        if (lossCounter > 0) {
          lossStreakCounts[lossCounter] =
            (lossStreakCounts[lossCounter] || 0) + 1
  ...
  function displayResults() {
    ...
    return (
      <div>
        <h2>Results</h2>
        <p>
          Out of <strong>{results.gameCount.toLocaleString()}</strong> eggs
          opened:
        </p>
        <ul>
          <li>
            Effective legendary drop rate:{' '}
            <strong>{results.actualWinRate}%</strong>
          </li>
          <li>
            Eggs with at least one legendary:{' '}
            <strong>{results.totalWins.toLocaleString()}</strong>
          </li>
          <li>
            Eggs with no legendaries:{' '}
            <strong>{results.totalLosses.toLocaleString()}</strong>
          </li>
          <li>
            Longest lucky streak: <strong>{results.bestWinStreak}</strong> eggs
            in a row with a legendary
          </li>
          <li>
            Longest dry streak: <strong>{results.worstLossStreak}</strong> eggs
            in a row with no legendaries
          </li>
        </ul>
  ...
  return (
    ...
    >
      <section id="eggs">
        ...
        <p>
          <em>
            &quot;Why didn&apos;t I get a legendary after 30 large eggs?! 3.4%
            chance my ass!&quot; --frustrated players
          </em>
        </p>
