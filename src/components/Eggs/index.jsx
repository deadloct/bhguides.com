...
export default function Eggs() {
  ...
  const calculateEggsAndChances = () => {
    ...
    const prices = {
      small: effectiveSmallPrice,
      large: effectiveLargePrice,
      mega: effectiveMegaPrice,
    }
  ...
  function displayResults() {
    ...
    return (
      <div>
        <h2>Results</h2>
        <p>
          Your drop rate of eggs with 1+ legendaries was{' '}
          <strong>{results.actualWinRate}%</strong>, successfully getting{' '}
          <strong>{results.totalWins}</strong> eggs with 1+ legendaries but
          striking out on <strong>{results.totalLosses}</strong>.
        </p>
        <p>
          Your worst streak of dud eggs was{' '}
          <strong>{results.losingStreaks['worst']}</strong>, but your best
          streak of eggs with 1+ legs was{' '}
              ...
              <TableBody>
                {resultsData.map((result) => (
                  <TableRow key={result.size}>
                    ...
                    <TableCell>
                      {result.totalCost}
                      {result.remainingGems > 0
                        ? ` (${result.remainingGems} left)`
                        : ''}
                    </TableCell>
