...
export default function FamiliarStats() {
  ...
  const addComparison = () => {
    setComparisons((prev) => [
      ...prev,
      { id: nextId, familiar: null, stableCount: 1 },
    ])
    setNextId((n) => n + 1)
  }
  ...
  return (
    ...
    >
      ...
      {disclaimerVisible && (
        <div className={`${styles['bubble']} ${styles['ext-link-risk']}`}>
          ...
          <div className={styles['bubble-message']}>
            ...
            <a href="https://bit-heroes.fandom.com/wiki">the wiki</a> on 29 June
            2026.
          </div>
          <WarningAmberIcon />
          <button
      ...
      <p>
        ...
        actual stat values. Add more familiars to compare them side by side.
      </p>
                ...
                >
                  ...
                  <Box sx={{ px: 1 }}>
                    <Typography
                      ...
                      gutterBottom
                    >
                      Stable: +{comparison.stableCount - 1} (
                      {(comparison.stableCount - 1) * 2}% bonus)
                    </Typography>
                    <Slider
                      value={comparison.stableCount}
                      onChange={(_, value) =>
                        updateComparison(comparison.id, { stableCount: value })
                      }
                      min={1}
                      max={6}
                      step={1}
                  ...
                  {stats ? (
                    ...
                    >
                      <StatRow
                        ...
                        pct={comparison.familiar.strength}
                      />
                      <StatRow
                        label="STA"
                        color="success.main"
                        value={stats.stamina}
                        pct={comparison.familiar.stamina}
                      />
                      <StatRow
                        label="AGI"
                        color="info.main"
                        value={stats.agility}
                        pct={comparison.familiar.agility}
                      />
                      <Typography
                        variant="caption"
                        color="text.secondary"
                      ...
                      >
                        ...
                        {stats.stableBonus > 0 &&
                          ` (+${Math.round(stats.stableBonus).toLocaleString()} stable)`}
                      </Typography>
                    </Box>
                  ) : (
                    <Typography
