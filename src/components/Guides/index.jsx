...
export default function Guides() {
  ...
  function inTierBadge(guide) {
    if (guide.inTier) {
      return (
        <>
          <span className={styles['in-tier-pill']} title="In-Tier Clear">
            IN-TIER
          </span>
  ...
  function obsolete(guide) {
    if (guide.obsolete && guide.obsolete.length) {
      return (
        <>
          <span className={styles['obsolete']} title={guide.obsolete}>
            Obsolete
          </span>
          <CancelIcon
  ...
  function authors(guide) {
    if (guide.authors && guide.authors.length) {
      ...
      return (
        <div>
          <em>{label}</em> {guide.authors.join(', ')}
        </div>
      )
    }
  ...
  function builds(guide) {
    if (guide.builds && guide.builds.length) {
      return (
        <div>
          <em>Builds:</em> {guide.builds.join(', ')}
        </div>
      )
    }
  ...
  function revealedBy(guide) {
    if (guide['revealed_by'] && guide['revealed_by'].length) {
      return (
        <div>
          <em>Revealed by:</em> {guide['revealed_by']}
        </div>
      )
    }
    ...
    switch (item.attachmenttype) {
      case 'image':
        return (
          <li
            key={`item-${i}`}
            className={`${styles['attachment-item']} ${styles['attachment-item-img']}`}
            onClick={() => openLightbox(item, anchor)}
          >
            <ImageIcon />
            <span className={styles['att-name']}>{item.filename}</span>
            <span
              className={styles['att-type']}
      ...
      case 'pdf':
        return (
          <li key={`item-${i}`} className={styles['attachment-item']}>
            <PictureAsPdfIcon />
            <a
              href={`/guide-files/${item.filename}`}
              target="_BLANK"
              rel="noreferrer"
            >
              {item.filename}
      ...
      case 'markdown':
        return (
          <li
            key={`item-${i}`}
            className={`${styles['attachment-item']} ${styles['attachment-item-markdown']}`}
            onClick={() => openMarkdown(item, anchor)}
          >
            <ArticleIcon />
            <span className={styles['att-name']}>{item.filename}</span>
      ...
      case 'video':
        return (
          <li
            key={`item-${i}`}
            className={`${styles['attachment-item']} ${styles['attachment-item-video']}`}
            onClick={() => openLightbox(item, anchor)}
          >
            <SmartDisplayIcon />
            <span className={styles['att-name']}>{item.filename}</span>
  ...
  function attachments(guide, anchor) {
    ...
    return (
      <ul>{guide.attachments.map((item, i) => attachment(item, i, anchor))}</ul>
    )
  }
  ...
  function renderGuide(guide, i, isSearch, catID) {
    ...
    if (isSearch && guide.categoryName) {
      cat = (
        <div>
          <em>Category:</em> {guide.categoryName}
        </div>
      )
    }
      ...
      >
        <div className={styles['guide-name']}>
          ...
          >
            ...
            {guide.inFestiviflux && (
              <AcUnitIcon
                titleAccess="Also available in Festiviflux Invasion"
                fontSize="small"
                className={styles['festiviflux-icon']}
              />
            )}
          </span>
          {inTierBadge(guide)}
  ...
  function renderGuidesForCategory(catID, cat, i, hasParent) {
    ...
    if (cat.guides.length > 0) {
      ...
      results = (
        <ul>
          {sortedGuides.map((g, i) => renderGuide(g, i, cat.isSearch, catID))}
        </ul>
      )
    } else {
  ...
  function renderCategoriesForIndex(indexEntry, i, depth = 0) {
    if (!indexEntry.children) {
      return renderGuidesForCategory(
        indexEntry.id,
        guidesData[indexEntry.id],
        i,
        depth > 0
      )
    }
    ...
    return (
      <div
        key={`${indexEntry.id}-${i}`}
        className={styles['category-branch-group']}
      >
  ...
  function renderTableOfContents() {
    const items = guideIndexData.map((item, i) => {
      ...
      if (item.children && item.children.length > 0) {
        children = item.children.map((child, i) => {
          ...
          return (
            <li className={styles['toc-child']} key={`toc-${i}`}>
  ...
  function render() {
    ...
    return (
      <>
        ...
        {...guideIndexData.map((indexEntry, i) =>
          renderCategoriesForIndex(indexEntry, i)
        )}
      </>
    )
  }
    ...
    <Container key="guides" className={styles['wrapper']} maxWidth="md">
      <header>
        <p className={styles['guides-description']}>
          Welcome to the #1 community repository of guides for Kongregate&apos;s
          Bit Heroes, created by dedicated players who graciously share their
          knowledge and strategies. Let us all dominate the jerk Porteriuz
          together!
        </p>
