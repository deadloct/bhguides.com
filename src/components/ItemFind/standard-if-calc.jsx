...
export default function StandardIFCalc() {
  ...
  useEffect(() => {
    const params = {
      ...
      daily:
        formValues.daily === 'thursday' && formValues.encounter === 'pvpTickets'
          ? 100
          : getOptionValue(options.daily, formValues.daily),
    }
        ...
        <Box mt={VerticalSpacing}>
          <FormControl fullWidth>
            ...
            >
              ...
              {Object.entries(options.consumables.groups['By Name']).map(
                ([key, v]) => (
                  <MenuItem key={key} value={key}>
                    {v.text}
                  </MenuItem>
              ...
              {Object.entries(options.consumables.groups['By Amount']).map(
                ([key, v]) => (
                  <MenuItem key={key} value={key}>
                    {v.text}
                  </MenuItem>
