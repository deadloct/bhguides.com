...
export default function TurnRateCalc() {
  ...
  function getInputHandler(name) {
    return (e) => {
      let v =
        e.target.value === ''
          ? limits[name].min
          : limit(parseFloat(e.target.value), name)
  ...
  return (
    ...
    >
        ...
        <Grid container justify="center" direction="column">
          <Grid>
            ...
            <Slider
              ...
              onChange={getSliderHandler('power')}
            />
          </Grid>
          <Grid>
            <Grid container justifyContent="space-between" direction="row">
            ...
            <Slider
              value={sliderValues.agility}
              min={limits.agility.min}
              max={limits.agility.max}
              valueLabelDisplay="auto"
              onChange={getSliderHandler('agility')}
            />
          </Grid>
          <Grid>
            <Grid container justifyContent="space-between" direction="row">
            ...
            <Slider
              ...
              step={0.1}
              valueLabelDisplay="auto"
              onChange={getSliderHandler('speed')}
            />
          </Grid>
        </Grid>
      </div>
      ...
      <p>
        Quote from the{' '}
        <a href="https://bit-heroes.fandom.com/wiki/Mechanics">BH Wiki:</a>
      </p>
      <blockquote cite="https://bit-heroes.fandom.com/wiki/Mechanics">
        As of 3/26/2017: Agility has been reworked. Turn rates are now based on
        a sum of power and agility. This will allow players to use different
        builds without losing any overall damage.
      </blockquote>
      ...
      <p>
        The more power your character has, the slower they will be. Turn rate is
        inversely proportional to your power because it&apos;s the denominator
        in the equation above.
      </p>
      <p>
        Speed bonuses act as a multiplier for your turn rate. For example if
        there was one build with 0% speed and another with exactly the same gear
        but with +100% speed, the second would take twice as many turns.
      </p>

      <h3>Credits</h3>
      <p>
        Thanks to Chocomint for helping with the text below this calculator.
        Also thanks to ChubbyDaemon and the wiki crew for providing the
        information and equation that it was based on, along with the turn rate
        svg.
      </p>
    </Container>
  )
